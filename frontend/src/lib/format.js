...
export const fmt = {
  /** 5,000 */
  int: (v) => (isNum(v) ? INT.format(Math.round(v)) : DASH),
  /** 1.58 — Sharpe and Fitness. */
  ratio: (v, digits = 2) => (isNum(v) ? v.toFixed(digits) : DASH),
  /** A fraction as a percent: 0.643 → 64.3% */
  pct: (v, digits = 1) => (isNum(v) ? `${(v * 100).toFixed(digits)}%` : DASH),
  /** A fraction as basis points: 0.0005 → 5.0 bps */
  bps: (v, digits = 1) => (isNum(v) ? `${(v * 10_000).toFixed(digits)} bps` : DASH),
  /** 12.4K */
  compact: (v) => (isNum(v) ? COMPACT.format(v) : DASH),
  /** 2h 05m, 4m 12s, 9s */
  duration: (seconds) => {
