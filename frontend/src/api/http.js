...
function fromObject(status, o) {
  const fields = o['fields']
  const fieldDetail = Array.isArray(fields?.['detail'])
    ? fields['detail'].map(String).join(' ')
    : text(fields?.['detail'])
...
export function normalise(status, raw) {
  if (raw && typeof raw === 'object') {
    const o = raw
    if (o['error'] && typeof o['error'] === 'object') return fromObject(status, o['error'])
    ...
    if (detail && typeof detail === 'object') return fromObject(status, detail)
