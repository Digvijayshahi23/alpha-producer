...
const setting = (key, header, width) => ({
  key,
  header,
  width,
  cell: (r) => <span className="num">{r.settings?.[key] ?? DASH}</span>,
})
/**
...
export function TasksScreen() {
  ...
  const total = (tasks, pick) => tasks.reduce((n, t) => n + pick(t), 0)
...
const RUN_LABEL = { IDLE: 'Run', PAUSED: 'Resume', FAILED: 'Retry' }
function Actions({ task, onAct, onEdit }) {
...
function TaskDetail({ task, onOpenAlpha }) {
  const top = useQuery({
...
function EditTask({ task, slots, onClose }) {
  const queryClient = useQueryClient()
