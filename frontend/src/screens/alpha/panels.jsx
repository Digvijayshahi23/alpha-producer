...
import { checkName, isCeiling, isQuickMode, matches, powerPoolRules, resultOf } from './analysis'
import { alpha as api } from './api'

// ── Figures ────────────────────────────────────────────────────────────────────────────────
/** A check's figure in its own unit: turnover as a percent, everything else as a ratio. */
const figure = (name, v) => (name.includes('TURNOVER') ? fmt.pct(v, 2) : fmt.ratio(v))
/**
...
export function VerdictPanel({ verdict, alpha, actions }) {
...
// ── Checks ─────────────────────────────────────────────────────────────────────────────────
export function ChecksPanel({ groups }) {
...
export function AggregatesPanel({ alpha }) {
...
const STAGE_LABEL = { TRAIN: 'Train', TEST: 'Test', OS: 'OS' }
export function YearlyPanel({ years, cutoff }) {
...
export function EligibilityPanel({ alpha }) {
 ...
 */
function useBudgeted(key, fetch) {
...
function CorrelationRow({ alphaId, kind, label, limit }) {
...
export function CorrelationsPanel({ alphaId }) {
  return (
    <Panel
      title="Correlations"
...
export function ComparisonPanel({ alphaId }) {
      ...
      <KV
        items={[
          ...
          ...(lineage.generation !== null && lineage.generation !== undefined
            ? [['Generation', fmt.int(lineage.generation)]]
            : []),
          ['Simulated', fmt.dateTime(lineage.simulatedAt)],
          ...params.map(([k, v]) => [k, typeof v === 'string' ? v : JSON.stringify(v)]),
        ]}
      />
      {lineage.lab === 'ga' && (
        <p className="text-body-compact text-ink-subtle">
