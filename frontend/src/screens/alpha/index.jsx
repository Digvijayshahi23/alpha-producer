 ...
 */
function RegionAgnosticNote({ type }) {
  if (type === 'RA_CHILD')
    return (
...
function Header({ alpha: a, fetchedAt }) {
...
function ExpressionPanel({ alpha: a }) {
...
function PerformancePanel({ view, cutoff, refresh }) {
  const [shown, setShown] = useState('pnl')
