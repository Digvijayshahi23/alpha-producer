...
import { submissionPlanner } from './api'

const fixed = (v, places = 2) => (v == null || !Number.isFinite(v) ? '—' : v.toFixed(places))
...
const lift = (d) => {
  ...
  return `${fixed((d.sharpe / d.bestSingle) * 100, 0)}% of the best single, spread over ${d.size} sources (${fixed(d.sharpe / mean)}× the average member)`
}
export function SubmissionPlannerScreen() {
  ...
  const mark = useMutation({
    mutationFn: ({ alphaId, submitted }) => submissionPlanner.setSubmitted(alphaId, submitted),
          ...
          {usable.map((t) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  setPicked((prev) => {
                    const next = new Set(prev.size ? prev : usable.map((u) => u.id))
