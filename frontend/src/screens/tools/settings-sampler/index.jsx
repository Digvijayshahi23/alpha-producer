...
import { pairLabel, settingsSampler } from './api'

/** A multi-simulation carries at most ten children, all sharing region and delay. */
const BATCH = 10
const pairKey = (p) => `${p.maxTrade}|${p.maxPosition}`
const MODES = [
  { value: 'expression', label: 'Expression' },
  { value: 'alpha', label: 'Alpha ID' },
]
const marketKey = (m) => `${m.region}|${m.delay}|${m.universe}`
...
const allMarkets = (plan) => plan.regions.flatMap((r) => r.markets)
 ...
 */
function GroupChip({ label, group, onChange, size = 'md', disabled }) {
  const { keys, on } = group
...
function Tree({ branches, onChange }) {
  return (
...
const clamp = (text, max) => Math.min(max, Math.max(0, Math.round(Number(text) || 0)))
/**
...
export function SettingsSamplerScreen() {
  ...
  const [mode, setMode] = useState(alphaId ? 'alpha' : 'expression')
  const [draft, setDraft] = useState(alphaId)
  const [expression, setExpression] = useState('')
  const [decay, setDecay] = useState('0')
  const [truncation, setTruncation] = useState('0.08')
  const [nanHandling, setNanHandling] = useState('ON')
  ...
  const [neutralizations, setNeutralizations] = useState([])
  ...
  const source = mode === 'alpha' ? (alphaId ? { alphaId } : null) : typed
  ...
  const add = useMutation({
    ...
    onError: (error) => toast.error('Could not add task', { description: errorMessage(error) }),
  })
  const analyse = (event) => {
