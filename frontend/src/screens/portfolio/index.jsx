...
/** `USA/D1/PV` → `USA / D1 / PV`. */
const spaced = (pyramid) => pyramid.split('/').join(' / ')
const FACETS = [
...
function metricColumns(get, width = 'minmax(84px,1fr)') {
...
export function PortfolioScreen() {
  ...
  const isIncluded = (m) => overrides.get(m.alphaId) ?? matches(m)
...
function FacetBlock({ facet, members, value, onChange }) {
...
const STATS_COLUMNS = [
  ...
  ...metricColumns((r) => r.stats),
]
function StatsTable({ result, windows }) {
...
function MembersTable({ rows, selected, onSelect, loading }) {
  const columns = [
    {
      key: 'id',
      header: 'Alpha',
    ...
    ...metricColumns((m) => m, 'minmax(96px,1fr)'),
  ]
  return (
    <DataTable
