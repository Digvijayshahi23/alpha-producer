...
const yearOf = (time) => (typeof time === 'object' ? String(time.year) : String(time).slice(0, 4))
const money = (price) =>
  new Intl.NumberFormat('en-US', { notation: 'compact', maximumFractionDigits: 2 }).format(price)
export function PortfolioChart({ dates, curve, testStart, label }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(node, {
      ...
      timeScale: {
        ...
        tickMarkFormatter: (time, type) => (type === TickMarkType.Year ? yearOf(time) : ''),
      },
      crosshair: {
    ...
    const points = dates.map((date, i) => ({ time: date, value: curve[i] ?? 0 }))
    const line = (name) =>
    ...
    chart
      .addSeries(
        BaselineSeries,
        {
          ...
          priceLineVisible: false,
          lastValueVisible: false,
          priceFormat: { type: 'custom', formatter: (p) => `${(p * 100).toFixed(2)}%` },
        },
        1,
      )
      .setData(underwater)
...
export function ChartLegend({ testStart }) {
  const swatch = (name, text) => (
