...
const signed = (render) => (v) => <span className={TEXT_TONE[signTone(v)]}>{render(v)}</span>
const COLUMNS = [
  {
    key: 'alpha_id',
    header: 'Alpha',
...
const unique = (values) => [...new Set(values.filter((v) => v != null))]
export function Stored({ onOpen }) {
  ...
  const [sort, setSort] = useState({ key: 'sharpe', desc: true })
