...
import { tokenizeBrainAst } from './brain-ast'
/** A check's value or limit: turnover as a percentage, the rest as a ratio. */
export const checkFigure = (name, v) => (name.includes('TURNOVER') ? fmt.pct(v) : fmt.ratio(v))
/** The strict submission boundary: submission happens on BRAIN, never here. */
export function OpenInBrain({ url }) {
  return (
...
export function RecheckButton({ alphaId }) {
  const queryClient = useQueryClient()
  const mutation = useMutation({
    mutationFn: () => pool.check(alphaId),
    onSuccess: (body) => {
      const checks = body.is?.checks ?? []
      const count = (r) => checks.filter((c) => (c.result ?? 'PENDING') === r).length
      toast.success(
...
export function AlphaActionsMenu({ alphaId }) {
...
export function AstInspector({ expression, className, allowCopy = true }) {
