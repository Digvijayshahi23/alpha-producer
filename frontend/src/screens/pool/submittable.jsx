...
const hiddenReasons = (data) =>
  [
    ...
    .filter(Boolean)
    .join('; ')
const pass = (ok) => (ok === null ? 'neutral' : ok ? 'profit' : 'loss')
export function Submittable({ onOpen }) {
...
function SettingsLine({ settings: s }) {
  const parts = [
    s.region,
    s.universe,
