...
import { llm, quotaDay } from '@/screens/ai/api'
...
export function Keys() {
  ...
  const remove = useMutation({
    mutationFn: (k) => llm.removeKey(k.id),
    onSuccess: (_, k) => {
  ...
  const today = (k) => k.usage.filter((u) => u.day === day)
  const columns = [
    {
      key: 'provider',
      header: 'Provider',
...
function EnabledCell({ apiKey: k }) {
  const invalidate = useInvalidateKeys()
  const toggle = useMutation({
    mutationFn: (enabled) => llm.setEnabled(k.id, enabled),
    onSuccess: (updated) => {
...
function CapCell({ apiKey: k }) {
  ...
  const save = useMutation({
    ...
    mutationFn: (cap) => llm.setEnabled(k.id, k.enabled, cap),
    onSuccess: (updated) => {
      toast.success(
...
function RowActions({ apiKey: k, onDelete }) {
  const invalidate = useInvalidateKeys()
