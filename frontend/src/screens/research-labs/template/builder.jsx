...
const tintOf = (category) => (category ? CATEGORY[category] : undefined)
/**
...
function useDragDrop(onDrop) {
  ...
  const targetOf = (event) => event.target.closest('[data-drop]')?.dataset['drop'] ?? null
  const clear = () => {
    setDragging(null)
    setOver(null)
  }
  const begin = (event, payload) => {
...
export function Builder({
  ...
  const change = (next) => onChange({ version: 1, root: next })
  const fits = (payload, path) =>
...
function SlotView({ slot, path, suffix }) {
...
function Floating({ open, onOpenChange, trigger, children }) {
  return (
...
function OperatorBlock({ node, path, suffix = '' }) {
  const ctx = useBuilder()
  const [open, setOpen] = useState(false)
  const state = useDropState(path)
  ...
  const tail = (position) => (position < count - 1 ? ',' : inline ? close : undefined)
  const arg = (index) => (
    <SlotView
      key={index}
...
function BlockMenu({ node, path, close }) {
  const ctx = useBuilder()
  const first = blockOf(ctx.blocks, node)
  const allowed = choices(node, ctx.blocks)
  const update = (next) => ctx.change(put(ctx.root, path, next))
 ...
 */
function WordOption({ name, fallback, value, onChange }) {
...
function OptionInput({ name, fallback, value, onChange }) {
...
function BlockSearch({ socket, onPick }) {
  const ctx = useBuilder()
  const sections = paletteSections(ctx.root, ctx.blocks, ctx.options)
    .map((section) => ({
      ...section,
