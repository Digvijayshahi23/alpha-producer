...
const EMPTY = { version: 1, root: null }
...
export function TemplateLabScreen() {
  ...
  const preview = useQuery({
    ...
    queryFn: () => templateLab.preview(JSON.parse(settledKey)),
  ...
  const saveAs = useMutation({
    mutationFn: (value) => templateLab.create({ ...value, tree: doc }),
    onSuccess: (row) => {
      draft.saved(Number(row.id), row.name)
      setNaming(null)
      void refreshTemplates()
  ...
  const rename = useMutation({
    ...
    mutationFn: (value) => templateLab.update(saved ?? 0, { ...value, tree: current?.tree ?? doc }),
    onSuccess: (row) => {
  ...
  const remove = useMutation({
    mutationFn: (id) => templateLab.remove(id),
    onSuccess: () => {
      setDeleting(false)
...
function TemplateGrid({ label, items, selected, dirty, onOpen }) {
...
function NameDialog({ title, initialName, initialDescription, pending, error, onClose, onSubmit }) {
