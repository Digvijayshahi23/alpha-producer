...
const LABEL = Object.fromEntries(LEGEND)
const NO_DELAY =
  'bg-[repeating-linear-gradient(135deg,var(--color-hairline-strong)_0_1px,transparent_1px_5px)]'
const keyOf = (region, delay, universe) => `${region}|${delay}|${universe}`
 ...
 */
function ownRun(run, regionAgnostic) {
...
export function SyncHero({ scope, onPick }) {
 ...
 */
export function RegionAgnosticHero({ scope, onPick }) {
  const markets = useQuery({
    queryKey: ['catalog', 'markets'],
    queryFn: catalog.markets,
    staleTime: 60 * 60 * 1000,
    ...
    >
      <div className="grid gap-2 sm:grid-cols-3">
        {universes.map((universe) => {
          const state = live.get(universe)?.state ?? (held.has(universe) ? 'done' : 'waiting')
...
function useDownload(regionAgnostic) {
  const queryClient = useQueryClient()
  return useMutation({
