...
const key = (categoryId, region, delay) => `${categoryId}|${region}|${delay}`
const times = (m) => (m == null ? DASH : `×${fmt.ratio(m, 1)}`)
 ...
 */
function standing(count, needed) {
  if (count >= needed) {
    return {
...
export function PyramidsScreen() {
  ...
  const [view, setView] = useState('multiplier')
