...
function Frame({ title, description, children, footer }) {
  return (
    <>
...
export function Dialog({ open, onOpenChange, className, ...frame }) {
  return (
...
export function Sheet({ open, onOpenChange, className, container, ...frame }) {
  return (
    <BDialog.Root open={open} onOpenChange={(next) => onOpenChange(next)}>
...
export function Select({ value, onChange, items, label, className, mono, disabled }) {
  return (
    <BSelect.Root
      items={items}
      value={value}
